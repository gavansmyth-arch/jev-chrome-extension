...
import { testTextModel } from '../bg/typer.js';
...
function fill(settings) {
  ...
  ui.tmProvider.value = textModel.provider;
  ui.tmModelName.value = textModel.model;
...
function read() {
  ...
  return {
    ...
    textModel: {
      ...
      model: ui.tmModelName.value.trim(),
    },
    drive: {
...
function applyPreset() {
  const preset = TEXT_MODEL_PRESETS[ui.tmProvider.value];
  if (!preset || ui.tmProvider.value === 'custom') return;
  ui.tmBase.value = preset.baseUrl;
  ui.tmModelName.value = preset.model;
}
